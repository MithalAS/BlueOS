...
class ChangeTimeoutResponse(BaseModel):
    timeout_updated: bool
    message: str
    serial_path: str
    timeout_seconds: int
...
class vescConfigEditor:
    ...
    def change_timeout(self, port: str, new_timeout_sec: int) -> ChangeTimeoutResponse:
        timeout: int = new_timeout_sec * 1000
        response_body: ChangeTimeoutResponse = ChangeTimeoutResponse(
            timeout_updated=False,
            message="",
            serial_path=port,
            timeout_seconds=new_timeout_sec,
        )
        for _ in range(3):
            try:
                ser: serial.Serial = serial.Serial(port, baudrate=115200, timeout=0.5)
                ...
                if response:
                    ...
                    response_body.timeout_updated = True
                    response_body.message = f"Timeout changed to {new_timeout_sec} seconds on port {port}."
                else:
                    response_body.message = f"No valid response received from VESC on port {port}."
                    logging.error(response_body.message)
                ser.close()
                return response_body
            except serial.SerialException as e:
                logging.error(f"Serial error: {e}")
                response_body.message = f"Serial error on port {port}: {e}"
            except ValueError as e:
                logging.error(f"Value error: {e}")
                response_body.message = f"Value error on port {port}: {e}"
            time.sleep(1)
        logging.error(f"Failed to change timeout after 3 attempts on port {port}.")
        return response_body

    def get_app_config(self, port: str, retries: int = 3, retry_delay: float = 0.5) -> Optional[AppConfigData]:
        """Get the app configuration from the VESC, with retries."""
        for attempt in range(retries):
            try:
                ...
                if response:
                    ...
                    if len(payload) < 278:
                        logging.error(f"Expected at least 278 bytes, got {len(payload)}")
                        return None
            ...
            time.sleep(retry_delay)
        logging.error(f"Failed to get valid VESC config after {retries} attempts on port {port}.")
        return None
